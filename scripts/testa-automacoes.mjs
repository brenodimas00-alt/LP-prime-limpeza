...
import { AGORA_TESTE, criarAvulso, criarDiaristaAprovada, chave } from './cenarios.mjs';
...
t.teste('ACEITE: confirmado -> avaliado com relógio avançado gera exatamente as notificações esperadas, na ordem', async () => {
  ...
  // antes da véspera: lembretes pendentes
  ...
  // cobrança do dia ainda não venceu (finalizado + 2h = 17h)
  ns = await notifs(amb);
  assert.equal(ns.find((n) => n.template === 'cobranca_dia').status, 'pendente');
  ...
  assert.deepEqual(cliente, [
    'pedido_recebido:simulada', 'entrada_confirmada:simulada', 'lembrete_vespera:simulada', 'diarista_a_caminho:simulada',
    'atendimento_iniciado:simulada', 'atendimento_finalizado:simulada', 'obrigado_avaliacao:simulada', 'cobranca_dia:simulada',
  ]);
...
t.teste('cancelar pedido cancela as agendadas e manda UMA mensagem de cancelamento', async () => {
  const amb = montarAmbiente(AGORA_TESTE);
  const api = amb.casos;
  const r = await api.confirmarAutoagendamento({ cliente: CLIENTE_EMPRESA, pacote: { tipoServico: 'empresarial', duracaoHoras: 4, metragem: 60, quantidadeDiarias: 4, frequencia: 'semanal' }, primeiraData: '2026-10-05', turno: 'manha' , conta: { senhaHash: 'a'.repeat(64) } }, { sessao: { ator: 'publico' }, chave: chave() });
  await api.confirmarPagamento(r.pagamentoEntradaId, { sessao: PRIME, chave: chave() });
  let ns = await notifs(amb);
...
t.teste('reagendar cancela o lembrete antigo e agenda o novo', async () => {
  ...
  const ns = (await notifs(amb)).filter((n) => n.template === 'lembrete_vespera');
...
t.teste('duplo clique (mesma chave) não duplica evento nem notificação', async () => {
  ...
  const k = chave();
  await Promise.all([
    api.confirmarPagamento(r.pagamentoEntradaId, { sessao: PRIME, chave: k }),
    api.confirmarPagamento(r.pagamentoEntradaId, { sessao: PRIME, chave: k }),
  ]);
  const ns = await notifs(amb);
  assert.equal(ns.filter((n) => n.template === 'entrada_confirmada').length, 1);
...
t.teste('GPT#2: reatribuição e cancelamento avisam a diarista que perdeu a diária', async () => {
  const amb = montarAmbiente(AGORA_TESTE);
  const api = amb.casos;
  const r = await api.confirmarAutoagendamento({ cliente: CLIENTE_EMPRESA, pacote: { tipoServico: 'empresarial', duracaoHoras: 4, metragem: 60, quantidadeDiarias: 2, frequencia: 'semanal' }, primeiraData: '2026-10-05', turno: 'manha' , conta: { senhaHash: 'a'.repeat(64) } }, { sessao: { ator: 'publico' }, chave: chave() });
  const d1 = await criarDiaristaAprovada(api);
  const d2 = await criarDiaristaAprovada(api);
