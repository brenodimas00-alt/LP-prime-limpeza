...
t.teste('IndexedDB: idempotência (mesma chave = mesmo pedido; conteúdo diferente = CONFLITO)', async () => {
  const r = await p.evaluate(async (raiz) => {
    ...
    const dados = { cliente: CLIENTE_RESIDENCIAL, pacote: { tipoLimpeza: 'padrao', metragem: 40, quantidadeDiarias: 1, frequencia: 'avulso', adicionais: [] }, primeiraData: data, turno: 'tarde' };
