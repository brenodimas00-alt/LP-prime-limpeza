...
t.teste('HTTP: status 201 na criação, 200 na repetição idempotente, 409 no conflito', async () => {
  const corpo = JSON.stringify({ cliente: (await import('./fixtures/seed.js')).CLIENTE_RESIDENCIAL, pacote: { tipoLimpeza: 'padrao', metragem: 50, quantidadeDiarias: 1, frequencia: 'avulso', adicionais: [] }, primeiraData: '2026-10-05', turno: 'manha' });
  ...
  const c = await fetch(`${baseUrl}/autoagendamentos`, { method: 'POST', headers: h, body: corpo.replace('"metragem":50', '"metragem":51') });
